...
| F-INIT (Install) | T-INST-5 | Walks repo bundles (authoring, session, monitor), asserts every SKILL.md is installed at `~/.claude/commands/<bundle>/<skill>.md` | Every SKILL.md exists at the expected commands path | Iterates source then checks installed path; falsifiable. | PASS |
